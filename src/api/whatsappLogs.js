...
export async function markWhatsappMessageRead(messageId) {
    try {
        ...
        const url = `${getApiUrl()}/webhook/whatsapp/mark-read/${messageId}`;
        const res = await fetch(url, {
            method: "POST",
            headers: {
                Accept: "application/json",
        ...
        if (!res.ok) {
            const errorData = await res.json().catch(() => ({}));
            throw new Error(errorData.message || "Could not mark message as read!");
        }
        return await res.json();
    } catch (err) {
        console.error('Error marking WhatsApp message as read:', err);
        throw new Error(err.message || "Failed to mark WhatsApp message as read");
    }
}